...
typedef struct ShortTermRPS {
    unsigned int num_negative_pics;//negative pics
    int num_delta_pocs;            //positive pics + negative pics = num_delta_pocs
...
typedef struct RefPicListTab {
    RefPicList refPicList[2];       //list0 and list1
} RefPicListTab;

typedef struct HEVCWindow {
    unsigned int left_offset;       //cropping window
...
typedef struct HEVCVPS {
    ...
    unsigned int vps_max_dec_pic_buffering[7];
    unsigned int vps_num_reorder_pics[7];
    unsigned int vps_max_latency_increase[7];
